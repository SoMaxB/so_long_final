...
int check_last_row(t_game *game)
{
    int i = 0;
    while (i < game->line_x) {
        if (game->map_cpy[game->line_y - 1][i] != WALL && game->map_cpy[game->line_y - 1][i] != '\n') {
...
int check_last_column(t_game *game)
{
    int i = 0;
    while (i < game->line_y) {
        int last_pos = strlen(game->map_cpy[i]) - 1;
...
{
    if (!check_first_row(game)) return 0;
    if (!check_last_row(game)) return 0;
    if (!check_first_column(game)) return 0;
    if (!check_last_column(game)) return 0;
    return 1;
}
